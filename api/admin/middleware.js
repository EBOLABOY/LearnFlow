  ...
  try {
    ...
    try {
      const [users] = await connection.execute(
        'SELECT id, email, role, is_active, last_login_at FROM users WHERE id = ? AND is_active = 1',
        [decoded.userId]
      );

      if (users.length === 0) {
        authStatus = 'user_not_found';
        logSecurityEvent('AUTH_FAILED', 'user_not_found_or_disabled', req, { userId });
        return res.status(401).json({
          success: false,
          message: '用户不存在或已被禁用'
        });
    ...
    logSecurityEvent('AUTH_ERROR', 'system_error', req, { 
      error: error.message,
      userId 
    });
 ...
 * 记录管理员操作日志
 */
async function logAdminAction(adminId, action, targetType, targetId = null, details = null, ipAddress = null) {
  try {
    const connection = await pool.getConnection();
    try {
